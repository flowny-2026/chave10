...
export default function GlobalSearch() {
  ...
  const [results, setResults]   = useState({ os: [], clientes: [], veiculos: [] });
  const [cache, setCache]       = useState({ os: [], clientes: [], veiculos: [] });
  ...
  const filtrar = useCallback((q) => {
    if (!q.trim()) { setResults({ os: [], clientes: [], veiculos: [] }); return; }
    ...
    setResults({ os, clientes, veiculos });
  }, [cache]);
  ...
  const total = results.os.length + results.clientes.length + results.veiculos.length;
  ...
  return (
    <div ref={wrapRef} style={{ position: 'relative', flex: 1, maxWidth: 420 }}>
      ...
      <div className="topbar-search" style={{ width: '100%' }}>
        ...
        {query && (
          <button
            onClick={() => { setQuery(''); setResults({ os: [], clientes: [], veiculos: [] }); inputRef.current?.focus(); }}
            ...
            <div style={{ maxHeight: 420, overflowY: 'auto' }}>
              ...
              {results.os.length > 0 && (
                <Section label="Ordens de Serviço">
                  {results.os.map(os => (
                    <button key={os.id} onClick={() => goTo('/app/os')} style={rowStyle}>
              ...
              {results.clientes.length > 0 && (
                <Section label="Clientes">
                  {results.clientes.map(c => (
                    <button key={c.id} onClick={() => goTo('/app/clientes')} style={rowStyle}>
              ...
              {results.veiculos.length > 0 && (
                <Section label="Veículos">
                  {results.veiculos.map(v => (
                    <button key={v.id} onClick={() => goTo('/app/veiculos')} style={rowStyle}>
